...
A detailed review was conducted to assess how well the trained models performed. The main metrics for evaluation were \textbf{Cohen's Kappa} and the \textbf{Area Under the Receiver Operating Characteristic Curve (AUC-ROC)}. Cohen's Kappa was chosen because it provides a robust measure of agreement that accounts for the possibility of the agreement occurring by chance, which is particularly useful for datasets with uneven class distributions. The AUC-ROC was used to evaluate the models' ability to rank and differentiate between classes.
