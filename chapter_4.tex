...
The Instagram subset underwent additional filtering to ensure data quality and coherence. Only posts containing both an image and a caption were retained; posts lacking either modality were discarded immediately. Furthermore, due to download errors in the original dataset, some image links were invalid or corrupted; such posts were also excluded from further processing.

After applying these content‑based filters, the number of users dropped from an initial 1,309 (the actual number of users in the CSV file of the PagkataoKo Instagram subset) to 752, and the total number of posts decreased from 195,757 to 139,713. This reduction reflects the removal of posts without captions, posts without accessible images, and any corrupted or unreadable image files.

To ensure a sufficient amount of data for reliable user‑level aggregation, only users with a minimum of 30 valid posts (each containing both an image and a caption) were retained. Conversely, to prevent users with extremely large posting histories from disproportionately influencing feature representations, a maximum of 300 of each user’s most recent posts was used for feature extraction. Users with more than 300 posts still contributed to the dataset, but only their 300 most recent posts were processed. The reported range of 30 to 1,740 posts refers to the total number of valid posts available per user in the filtered dataset; however, for any user exceeding 300 posts, only the most recent 300 were actually used for feature extraction

Additionally, due to constraints in the original PagkataoKo data collection process (API limitations and download errors), each post in the final dataset contains exactly one image. Posts that originally contained multiple images were reduced to the first image only at the time of collection, and video posts were discarded entirely (Tighe et al., 2022).
...
All posts in this final dataset contain exactly one image and one textual caption. The 300‑post limit ensures that users with extensive posting histories do not disproportionately influence feature representations, while the 30‑post minimum ensures sufficient data for reliable user‑level aggregation.
...
\begin{table}[h]
	...
	\label{tab:class_label_distribution}
	\begin{tabular}{lcc}
		\hline
		\textbf{Personality Trait} & \textbf{"High" Count} & \textbf{"Low" Count} \\
		\hline
		Openness & 319 (42.42\%) & 433 (57.58\%) \\
		Conscientiousness & 359 (47.74\%) & 393 (52.26\%) \\
		Extraversion & 375 (49.87\%) & 377 (50.13\%) \\
		Agreeableness & 367 (48.80\%) & 385 (51.20\%) \\
		Neuroticism & 352 (46.81\%) & 400 (53.19\%) \\
		\hline
	\end{tabular}
\end{table}
...
For model training and evaluation, the dataset was divided using a user-level stratified split: \textbf{70\%} for training, \textbf{15\%} for validation, and \textbf{15\%} for testing. This split is used for all models and traits. The rationale for this decision is that a user-level split ensures that users' data remains completely "unseen" prior to validation or testing. To ensure that the proportions of High and Low labels for each personality trait remained consistent across these sets, the split was performed using a multi-label, iterative stratification based on the generated personality labels. This approach helped prevent model bias and ensured that the models were trained and evaluated on representative distributions of each personality class. Table \ref{tab:tvt_distribution} presents the distribution of low and high labels for each personality trait in each set.

\begin{table}[h]
\centering
\caption{Dataset Split Distribution by Personality Trait}
\label{tab:tvt_distribution}
\begin{tabular}{llccc}
\hline
\textbf{Trait} & \textbf{Set} & \textbf{Count} & \textbf{High (\%)} & \textbf{Low (\%)} \\
\hline

\multirow{3}{*}{Openness}
 & Train & 526 & 42.40\% & 57.60\% \\
 & Validation & 112 & 42.86\% & 57.14\% \\
 & Test & 114 & 42.11\% & 57.89\% \\
\hline

\multirow{3}{*}{Conscientiousness}
 & Train & 526 & 47.72\% & 52.28\% \\
 & Validation & 112 & 47.32\% & 52.68\% \\
 & Test & 114 & 48.25\% & 51.75\% \\
\hline

\multirow{3}{*}{Extraversion}
 & Train & 526 & 49.81\% & 50.19\% \\
 & Validation & 112 & 50.00\% & 50.00\% \\
 & Test & 114 & 50.00\% & 50.00\% \\
\hline

\multirow{3}{*}{Agreeableness}
 & Train & 526 & 48.68\% & 51.14\% \\
 & Validation & 112 & 49.11\% & 50.89\% \\
 & Test & 114 & 48.25\% & 51.75\% \\
\hline

\multirow{3}{*}{Neuroticism}
 & Train & 526 & 46.77\% & 53.23\% \\
 & Validation & 112 & 46.43\% & 53.57\% \\
 & Test & 114 & 47.37\% & 52.63\% \\
...
The demographic distribution of the final filtered 752-user subset was summarized in Table~\ref{tab:demo}. The data showed a user base that was predominantly young (51.9\% aged 18–20) and female (74.6\%).

Referencing the original work of \citet{tighe_acorda_2022} on this dataset, the language of each post was determined using a majority vote between two language identifiers (Polyglot and FastText). For this analysis, posts whose identifiers agreed were labeled as either English (51.3\%) or Tagalog (2.6\%) if those were the determined languages. Posts where the identifiers did not agree, but at least one identifier classified the language as either English or Tagalog, were assigned a ‘Conflict’ label (29.3\%), a category understood to primarily represent posts containing code-switching or other forms of mixed-language use. Posts where neither identifier classified the language as either English or Tagalog were assigned an ‘Other’ label (16.8\%).

\begin{table}[h]
	\centering
	\caption{Demographic distribution of filtered Instagram subset (n=752)}
	\label{tab:demo}
	\begin{tabular}{lcc}
		\hline
		\textbf{Characteristic} & \textbf{Category} & \textbf{Percentage} \\ \hline
		\textbf{Age} & 18-20 & 51.9\% \\
		& 21-23 & 29.7\% \\
		& 24-26 & 10.3\% \\
		& $\geq$27 & 6.4\% \\ 
& Not specified & 1.7\% \\ \hline
		\textbf{Sex} & Female & 74.6\% \\
		& Male & 19.7\% \\
		& Intersex/Declined & 5.7\% \\ \hline
		\textbf{Language} & English & 51.3\% \\
		& Tagalog & 2.6\% \\
		& Conflict (English/Tagalog) & 29.3\% \\ 
& Other & 16.8\% \\ \hline
	\end{tabular}
\end{table}
...
\subsection{Managing Feature Dimensionality}
Dimensionality reduction was explored in both TF-IDF and Imagga although it was only used for TF-IDF features. 

The TF-IDF vectorizer, implemented using the scikit-learn library, was configured with the max\_features parameter set to 1,500. This parameter instructs the vectorizer to build a vocabulary that only includes the 1,500 most frequent terms across the entire corpus.

For the Imagga tags, a similar reduction was explored by applying a confidence threshold to filter out low-confidence detections. This approach aimed to retain only the most reliably detected objects and scenes in each image. However, the validation performance of the confidence-filtered features proved negligible across all personality traits, leading to their exclusion from the final feature sets. Consequently, the full set of Imagga tags was retained for subsequent experiments.
...
\begin{itemize}
    ...
    \item \textbf{Word2Vec:} Two aggregation strategies—\textit{words‑weighted‑equally} (or coded as \textbf{WWE}, averaging all word vectors across all captions) and \textit{posts‑weighted‑equally} (or coded as \textbf{PWE}, averaging per post then across posts), as described in Section~\ref{subsec:wordembeddingrepresentation}.
