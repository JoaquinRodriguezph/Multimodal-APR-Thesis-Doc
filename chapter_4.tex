...
The Instagram subset underwent additional filtering to ensure data quality and coherence. The filtered out subset were left in the original dataset, while the subset that satisfies the filtering condition were saved in a new dataset. Only users with a minimum of 30 posts containing both images and captions were retained. Additionally, a maximum of 300 of each user's most recent posts were included in the final analysis.
...
The Imagga tags were utilized for feasibility since they were pre-included in the provided PagkataoKo Dataset, eliminating the need for independent processing.
...
The methodological approach of this study prioritized maintaining the highest possible level of interpretability by utilizing the full feature vectors generated from ResNet50 (2,048 dimensions) and the linguistic models. Consequently, no dimensionality reduction techniques, such as Principal Component Analysis (PCA), were applied to any of the extracted features.

By avoiding PCA, the study ensures that every feature remains in its original high dimensional space, allowing model predictions to be traced back to specific visual cues or textual tokens such as a particular object identified by ResNet50 or a specific word in a caption. To manage the complexity of the textual modality while preserving this interpretability, the TF-IDF feature set was limited to a fixed vocabulary of the top 1,500 most significant features based on term frequency across the corpus. This approach allows the models to operate on a robust and manageable subset of the linguistic data without the loss of semantic clarity that would result from transforming the features into abstract principal components.
