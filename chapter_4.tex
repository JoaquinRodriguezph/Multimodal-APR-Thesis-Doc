...
This chapter outlined the research method designed to create and assess a multimodal Automatic Personality Recognition (APR) framework for Filipino Instagram users. This framework addressed the specific language and cultural challenges found in the dataset, including code switching and unique patterns of visual self-expression. The process started with preprocessing, followed by exploratory data analysis and feature extraction from text, images, and metadata. It also included an early combination of these features. Finally, machine learning models were trained and thoroughly evaluated to determine how well they performed in predicting users’ personality traits.
...
This pipeline processed all valid images from a user's profile to extract a comprehensive set of visual features. For each image, the following features were extracted in parallel before being aggregated to the user level.
...
Furthermore, the used of Imagga is also for feasibility due to the Imagga JSON tags already being a part of the PagkataoKo Dataset.
...
Features were extracted from each image via three parallel processes: High-Level Semantics (ResNet50), Object Content (Imagga API), and Low-Level Aesthetics. These post-level features were then aggregated to the user level and concatenated. The final user-level image vector was a combination of these three feature sets, with an estimated total dimensionality of approximately 2,269 (2,048 from ResNet50, ~200 from top Imagga tags, and 21 from aesthetic features).
...
A user's entire caption corpus was processed through two parallel paths to extract lexical features (TF-IDF) and semantic embeddings (Word2Vec). These features were aggregated to the user level and concatenated. The final output was a user-level text vector, with a dimensionality equal to the sum of the Word2Vec dimension (300) and the training set’s vocabulary size (V) from TF-IDF.
...
Following feature extraction, a feature selection step was implemented to identify relevant predictors. The approach varied based on the feature type.

For discrete, categorical features, such as the tags generated by the Imagga API, the \textbf{Chi-Squared ($\chi^2$) test of independence} was used. This statistical method was well-suited for evaluating the association between each categorical feature and the binary High/Low personality labels. Features that showed a statistically significant association (indicated by a low p-value) were considered highly relevant.

For the high-dimensional, dense features generated by ResNet50 and BERT, the Chi-Squared test was not appropriate. Instead, the relevance of these features was assessed post-modeling using the interpretability techniques described in Section \ref{subsec:analysis}, such as SHAP. This allowed for an evaluation of feature importance based on their actual contribution to the final model predictions.
...
A detailed review was conducted to assess how well the trained models performed. The main metrics for evaluation were the \texttt{Macro-F1 score} and the \texttt{Area Under the Receiver Operating Characteristic Curve (AUC-ROC)}. The Macro-F1 score was chosen because it works well for datasets with uneven class distributions. The AUC-ROC was used to evaluate the models’ ability to rank and differentiate between classes.

\texttt{SHAP (SHapley Additive exPlanations)} was employed to understand how the models made decisions. SHAP allowed for the measurement of how much each feature contributed to the models’ predictions. This generated plots showing global feature importance, providing a clear ranking of the most impactful visual, textual, and metadata cues for each personality trait.
