...
\subsection{Preliminary Configuration Analysis and Ablation Studies}

To ensure a clear distinction between experimental development and final evaluation, the following discussion addresses the preliminary configuration and ablation studies conducted during the model-building phase. These experiments were performed exclusively on the training and validation data to refine the feature engineering pipeline and hyperparameter settings before the models were applied to the unseen test set. By isolating these tests, we provide a more transparent view of the architectural choices that informed the final multimodal framework.

One of the primary areas of investigation involved determining the optimal dimensionality for the textual features, specifically concerning the TF-IDF vocabulary size and Word2Vec aggregation methods. We evaluated whether a full vocabulary of over 36,000 terms or a more concise selection of high-frequency terms better captured the linguistic nuances of the Filipino Instagram users. Similarly, the choice between "All words equal" and "All posts equal" pooling strategies was determined on a trait-by-trait basis, selecting the method that yielded the highest Cohen’s Kappa on the validation set. This iterative process allowed us to tailor the semantic representation of each personality dimension, such as prioritizing dense semantic vectors for Extraversion while relying on broader vocabulary counts for Conscientiousness.

Furthermore, ablation studies were conducted on the visual and metadata components to mitigate noise within the multimodal fusion. This included testing various confidence thresholds for the Imagga tags to ensure that only the most reliable object and scene descriptors were integrated into the model. We also examined the impact of different feature-level fusion combinations—such as combining ResNet50 embeddings with account metadata—to identify which pairings provided the most stable performance across the validation folds. These preliminary findings served as the foundation for the finalized models, ensuring that the results reported in the following sections represent the most robust configurations for predicting the Big Five traits in a Philippine social media context.

\subsection{Model Strengths and Limitations by Trait}
An analysis of where the models succeeded or failed reveals that predictive performance is highly dependent on how individual personality traits manifest through social media. The results indicate that while certain traits are easily captured through digital footprints, others remain elusive regardless of the feature fusion used.

The models performed most reliably on Extraversion and Openness. These traits appear to have the most explicit markers on Instagram, where high-extraversion users tend to post more frequently and use diverse visual content. The success of the ResNet50 and Word2Vec fusion suggests that the combination of social presence and expressive language provides a rich signal for these dimensions. Similarly, Openness was effectively modeled by capturing the breadth of vocabulary and the variety of visual themes such as art, travel, and nature identified by the Imagga tags.

In contrast, Neuroticism and Agreeableness proved much harder to predict, often yielding the lowest Cohen’s Kappa scores. This suggests a inherent limitation in the dataset; these traits are fundamentally tied to internal emotional states and interpersonal tendencies that may not be explicitly shared in public-facing posts. While textual features like TF-IDF provided some signal for Conscientiousness likely by capturing structured and formal language patterns—the models often struggled to find a "visual" equivalent for these more private traits.

A recurring issue across the best-performing models was the impact of class imbalance. The models frequently defaulted to predicting the majority class (typically "High" or "Average"), leading to higher error rates for the "Low" trait labels. This reveals that while our multimodal approach is effective at identifying strong personality signals, the models currently lack the sensitivity to distinguish between more subtle or less frequent personality expressions. This performance gap highlights the challenge of using automated recognition in a real-world, unbalanced dataset like PagkataoKo, where the distribution of human personality is not uniform.
...
There is a 0.0000 gap between the top SVM and Logistic Regression performance, with the top SVM model performance utilizing only Word2Vec PWE features and the top Logistic Regression model performance using both Word2Vec WWE features and HSV. Correctly classified users had a higher average post count (213.45) compared to incorrectly classified users (145.17). It was also observed that misclassified users often utilized single-emoji captions or very short text, which might limit the model's performance due to lack of text.
...
Analysis of descriptive statistics revealed that correctly classified users had a significantly 
...
and warm colors. This suggests that specific aesthetic color choices are strong 
differentiators for this trait.
...
Descriptive statistics showed that correctly classified users averaged more total posts 
...
Descriptive statistics showed that misclassified users displayed significantly higher 
