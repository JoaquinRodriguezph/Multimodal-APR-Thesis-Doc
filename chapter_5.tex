...
Extraversion achieved its best result ($\kappa = 0.3684$) with Word2Vec PWE using the SVM model.
...
Openness achieved its best result ($\kappa = 0.2762$) through the ResNet50 + HSV combination using the Logistic Regression model. The closest competitor was Logistic Regression on U4 (ResNet50‑only), achieving $\kappa = 0.2602$ (gap $0.0160$).
...
For Conscientiousness, the TF-IDF, Word2Vec, and Metadata combination ($\kappa = 0.2451$) garnered the best kappa score, using the SVM model. The two closest competitors were SVM on TF‑IDF + Word2Vec + HSV and SVM on TF‑IDF + Word2Vec + HSV + Metadata, both achieving $\kappa = 0.2442$ with a gap of only $0.0009$.
...
Agreeableness achieved its best result ($\kappa = 0.2129$) through the Word2Vec + Metadata combination using the SVM model. The closest competitors were XGBoost on TF‑IDF + Word2Vec + HSV with $\kappa = 0.2091$ (gap $0.0038$) and XGBoost on TF‑IDF + Word2Vec with $\kappa = 0.2081$ (gap $0.0048$).
