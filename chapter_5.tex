...
Extraversion emerged as the most predictable trait in this study ($\kappa = 0.3684$). The best combination utilized Word2Vec PWE (Posts Weighted Equally) features, with Support Vector Machine as the optimal model.
...
Openness achieved its best result ($\kappa = 0.2762$) through the U8 combination (ResNet50 + HSV) using the Logistic Regression model.
...
For Conscientiousness, the B24 (TF-IDF, Word2Vec, and Metadata) combination ($\kappa = 0.2451$) garnered the best kappa score, using the SVM model.
