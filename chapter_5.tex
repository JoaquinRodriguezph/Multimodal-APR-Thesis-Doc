...
The complete failure of the 30\% confidence filter provides further evidence. By removing tags with confidence below 30\%, we inadvertently eliminated most tags entirely, leaving feature vectors that were predominantly zero. When combined with TF-IDF's IDF weighting---which can also zero out terms appearing in every document---the filtered tags produced matrices with insufficient non-zero entries for models to learn any signal, resulting in the 0 kappa scores observed in both validation and test sets.
