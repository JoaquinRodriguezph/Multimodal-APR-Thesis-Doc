...
Textual data from Instagram captions provides valuable linguistic cues for personality inference. This study employs the \textit{Vector Space Model} \citep{salton1975}, where text is represented as weighted feature vectors using the Term Frequency Inverse Document Frequency (TF-IDF) scheme. TF-IDF reflects the importance of words within individual captions while downweighting common terms across the corpus, enhancing the model's ability to capture personality-relevant language patterns.


TF-IDF is a feature selection model that "measures how often a word was used by a given personality type as compared to all the personality types." It is best used in "informal text data" as it takes into account the whole body of text when measuring the frequency of the word given the personality type. \citep{Pradhan_Bhansali_Chandnani_Pangaonkar_2020}.
...
Word2Vec learns dense vector representations of words, capturing semantic and syntactic meanings based on training on large text corpora. It provides accurate representations at lower computational costs with a simpler architecture, which is ideal for larger datasets such as the one we worked with.  \citep{Ma_Zhang_2015}. It can model “linguistic regularities and patterns” \citep{Mikolov_Sutskever_Chen_Corrado_Dean_2013}, which is useful for the study, as the dataset that was used contains informal language, slang, typos, and morphologically rich Tagalog words. 
...
According to \citet{Almodawar_Ahmad_Alqahtani_2025}, padding is important in the filter size requirement of CNNs. Without padding, the resulting output feature map would result an image with a different size from the input image. Zero-padding is one of the padding techniques that employ zero values on the extra pixels. \citet{Almodawar_Ahmad_Alqahtani_2025} stated that the advantages of zero-padding are simple implementation, neutral background for learning, and beneficial for edge detection and image segmentation. Another advantage in using image padding compared to other methods such as cropping and image stretching is padding reduces the loss of information. 

While this study uses padding only, other preprocessing methods such as center-cropping and stretching are valid as they solve the non-square aspect ratio problem. Center-cropping crops at the center of the image, which is where the subject usually is. Stretching uses image interpolation in scaling the image to a square aspect ratio.
...
Once all $\chi^2$ values are obtained, they will be summed to get the final $\chi^2$ value for the whole table. In order to determine the significance of this statistic, the \textit{degrees of freedom} is calculated using this formula:
