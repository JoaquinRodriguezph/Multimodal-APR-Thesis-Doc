...
\begin{table}[h]
	\centering
	\caption{Summary of Top Performing Models and Feature Sets per Trait}
	\label{tab:conclusion_results}
	\begin{tabular}{lllc}
		\hline
		\textbf{Trait} & \textbf{Model Code} & \textbf{Primary Features} & \textbf{Cohen's $\kappa$} \\ 
		\hline
		Openness & U8 (LR) & ResNet50 + HSV & 0.2762 \\
		Conscientiousness & B24 (SVM) & TF-IDF + Word2Vec + Metadata & 0.2451 \\
		Extraversion & B10 (LR) & Word2Vec + HSV & 0.3684 \\
		Agreeableness & M10 (SVM) & Word2Vec + HSV + Metadata & 0.1979 \\
		Neuroticism & TF-IDF (SVM) & Textual (All) & 0.1615 \\
		\hline
	\end{tabular}
\end{table}
