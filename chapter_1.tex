...
\begin{itemize}
	\item \textbf{Text Features}: The scope includes processing captions in \textbf{all languages} present in the dataset, acknowledging that code-switching between English, Filipino, and other languages is a natural characteristic of Filipino social media use. While all languages are retained, the primary analytical focus is on English and Filipino, as they are the most prevalent. To preserve expressive context, social media artifacts like emojis, hashtags, and links will be retained as meaningful tokens rather than removed. 
	
	It will utilize \textbf{TF-IDF} and pre-trained \textbf{Word2Vec embeddings}. While more advanced contextual models like BERT exist, they are deliberately excluded from this study's scope for two primary reasons. First, this research prioritizes \textbf{interpretability}; TF-IDF allows for a direct analysis of which specific words contribute most to personality prediction, aligning with the objective of identifying key linguistic markers. Second, this choice establishes as a strong baseline for the complex Filipino linguistic context \citep{tighe_acorda_2022}, allowing the study to focus on the effectiveness of multimodal fusion—rather than the optimization of a single complex language model.
	\item \textbf{Image Features}: The scope is defined by three  feature types:
	\begin{itemize}
		\item \textbf{High-Level Semantic Features}: A pre-trained \textbf{Resnet50}  Model will be used to extract deep feature representations from images. These neural-based features capture abstract visual concepts, textures, and compositional styles that are difficult to quantify with other methods.
		\item \textbf{Content-Based Features}: The \textbf{Imagga API} will be utilized to identify and quantify the presence of specific objects, scenes, and people (e.g., selfies, group photos, food, nature). This is based on the rationale that the content users choose to share reflects their activities and interests, which have been linked to traits like Extraversion \citep{Barry2019, Danciu2021, celli_automatic_2014, ferwerda2016}  and Openness \citep{Guntuku2017, Silvia2015} . 
	\item \textbf{Low-Level Aesthetic Features}: The model will extract quantitative measures of an image's aesthetic properties, including \textbf{color distributions} (hue, saturation) and \textbf{brightness}. Prior research indicates these visual characteristics correlate with specific personality traits. For instance:
	\begin{itemize}
