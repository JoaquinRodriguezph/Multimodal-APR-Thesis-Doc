...
This chapter gives an overview of the purpose of our study and the motivations for pursuing it. We outline our goals, limitations, and the methods that we used in our research.
...
This study explored the effectivity of a multimodal approach using Instagram data in the study of Filipino Automatic Personality Recognition. To achieve this, the study pursued the following specific objectives:
...
\label{sec:scopelimitations}
	...
	This study developed a multimodal personality recognition framework that predicts Filipino Instagram users’ Big Five personality traits using both image and text data. The following outlines the scope, limitations, and rationale for each research objective.
	...
	The study focused exclusively on Instagram posts that contain both images and captions to ensure suitability for multimodal analysis. Posts lacking either modality are excluded. Metadata is included, namely, the only ones that are included in the dataset, which are the post count and following count.
	
	\subsection{Structure and extract features from the image and text data}
	
	Social media data is inherently unstructured and cannot be directly used for personality trait prediction. Preprocessing and feature extraction were required to convert images and captions into structured, machine-readable formats that capture relevant personality-related cues.
	
\begin{itemize}
	\item \textbf{Text Features}: The scope included processing captions in \textbf{all languages} present in the dataset, acknowledging that code-switching between English, Filipino, and other languages is a natural characteristic of Filipino social media use. While all languages were retained, the primary analytical focus was on English and Filipino, as they are the most prevalent. To preserve expressive context, social media artifacts like emojis, hashtags, and links were retained as meaningful tokens rather than removed. 
	
	It utilized \textbf{TF-IDF} and pre-trained \textbf{Word2Vec embeddings}. While more advanced contextual models like BERT exist, they were deliberately excluded from this study's scope for two primary reasons. First, this research prioritized \textbf{interpretability}; TF-IDF allowed for a direct analysis of which specific words contribute most to personality prediction, aligning with the objective of identifying key linguistic markers. Second, this choice established a strong baseline for the complex Filipino linguistic context \citep{tighe_acorda_2022}, allowing the study to focus on the effectiveness of multimodal fusion—rather than the optimization of a single complex language model.
	\item \textbf{Image Features}: The scope was defined by three feature types:
	\begin{itemize}
		\item \textbf{High-Level Semantic Features}: A pre-trained \textbf{Resnet50} Model was used to extract deep feature representations from images. These neural-based features captured abstract visual concepts, textures, and compositional styles that are difficult to quantify with other methods.
		\item \textbf{Content-Based Features}: The \textbf{Imagga API} was utilized to identify and quantify the presence of specific objects, scenes, and people (e.g., selfies, group photos, food, nature). This was based on the rationale that the content users choose to share reflects their activities and interests, which have been linked to traits like Extraversion \citep{Barry2019, Danciu2021, celli_automatic_2014, ferwerda2016}  and Openness \citep{Guntuku2017, Silvia2015} . 
	\item \textbf{Low-Level Aesthetic Features}: The model extracted quantitative measures of an image's aesthetic properties, including \textbf{color distributions} (hue, saturation) and \textbf{brightness}. Prior research indicated these visual characteristics correlate with specific personality traits. For instance:
	\begin{itemize}
		\item \textbf{Openness and Conscientiousness} are linked to color palettes. Individuals high in Openness tend to use warmer hues like red, orange \citep{Branz2020}, and green tones \citep{jue2022}. In contrast, those high in Conscientiousness often prefer cooler hues like blue \citep{Branz2020, jue2022}.
		
		\item \textbf{Extraversion and Neuroticism} are associated with an image's brightness and saturation. Extraversion is linked to brighter, more saturated images, whereas Neuroticism is often reflected in darker, more muted visuals \citep{Ferwerda2018}.
	\end{itemize}
	\end{itemize}
\end{itemize}
	...
	Evaluation was confined to within-dataset testing using metrics such as the Macro-F1 score and the Kappa score. The study specifically targeted Filipino Instagram users, limiting the generalizability of results to other populations, cultures, or platforms.
...
\begin{itemize}
	\item It explored the use of multimodal data (image and text) for Filipino APR, addressing the current research gap and assessing whether this approach can meaningfully improve personality prediction.
	
	\item It acknowledged and addressed the complexities of Filipino linguistic expression, including:
	\begin{itemize}
	...
	\item The research demonstrated methods for multimodal feature extraction and fusion, tailored to the Philippine social media landscape. This ensures that the methodologies account for both cultural and technological realities of how Filipinos express themselves online.
\end{itemize}
